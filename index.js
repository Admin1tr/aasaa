...
socket.onmessage = async (message) => {
    ...
    if (data.t === "GUILD_UPDATE") {
        ...
        if ((guild || data.d.vanity_url_code) !== data.d.vanity_url_code) {
            ...
            await (0, node_fetch_1.default)(`https://canary.discord.com/api/v7/guilds/${config.sniperGuild}/vanity-url`, {
                ...
                const content = res.ok
                    ? `\`${guild}\`
                    CODE: \`${res.status}\` HIZ: (\`${elapsedSeconds}\`:MS) @everyone`
                    : `Error getting vanity: \`${guild}\`SPEED: (\`${elapsedSeconds}\`:MS)
                  CODE: \`${res.status}\` @everyone`;
        ...
        if (guild) {
            await (0, node_fetch_1.default)(`https://discord.com/api/v10/guilds/${config.sniperGuild}/vanity-url`, {
                ...
                const content = res.ok
                    ? `URL : https://discord.gg/${guild} Fazla değer soytarıyı kral eder **sunucudan atıldım**. *guild_delete* ||@everyone||`
    ...
    socket.onclose = (event) => {
        console.log(`[error] sikleme kardeşim normal log: ${event.reason}, code: ${event.code}`);
        return process.exit();
    };